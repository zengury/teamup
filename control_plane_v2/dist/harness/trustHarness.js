...
export function assessArtifactTrust(input) {
    const signals = [
        ...
        },
        {
            ...
            weight: 0.18,
            reason: input.context.sourceArtifacts.length > 0
...
export function downstreamReviewToVerification(review) {
...
export function downstreamLearningSignal(review) {
...
export function trustGate(assessment) {
...
export function roleTrustGate(input) {
    ...
    const missingRequiredSignals = spec.trustGate.requiredSignals.filter((kind) => input.assessment.signals.every((signal) => signal.kind !== kind || !signal.passed));
...
export function attachTrustToArtifactBody(body, trust) {
    return {
        ...body,
        trust: {
            score: trust.score,
...
function hasSourceCitations(body) {
    return Boolean(body.sourceContext) || Array.isArray(body.sourceCitations);
}
function hasDeclaredAssumptions(body) {
...
function weightForVerification(kind) {
    if (kind === "customer_confirmation")
        return 0.28;
    if (kind === "shaseng_eval")
        return 0.24;
    if (kind === "tangseng_decision")
        return 0.2;
    if (kind === "downstream_review")
        return 0.18;
    if (kind === "upstream_review")
        return 0.16;
    return 0.1;
}
function actionForMissingSignal(kind) {
    if (kind === "source_citations_present")
        return "Add source citations or sourceContext.";
    if (kind === "source_artifacts_present")
        return "Attach durable source artifacts.";
    if (kind === "assumptions_declared")
        return "Declare assumptions and uncertainties.";
    if (kind === "tool_result_attached")
        return "Attach tool-backed evidence where possible.";
    if (kind === "schema_valid")
        return "Repair artifact schema.";
    if (kind === "downstream_review")
        return "Record downstream learning signal and adjust future upstream confidence.";
    return `Resolve missing trust signal: ${kind}.`;
}
function round(value) {
    return Math.round(value * 100) / 100;
}
function averageDimensionScore(review) {
