export function createJourney(engagementId) {
    return {
        engagementId,
        state: "idle",
        lastArtifactIds: [],
...
function handleIdle(current, event) {
    if (event.type !== "conversation.transcribed")
        return stay(current);
    return {
...
function handleConversationReceived(current, event) {
...
function handleIntakeReady(current, event) {
...
function handleTangsengPlanning(current, event) {
    if (event.type === "tangseng.brief.created") {
...
function handleFounderApproval(current, event) {
    ...
    if (event.type !== "founder.approved")
        return stay(current);
    return startExecutionAfterFounderApproval(current, event.artifactIds);
}
function handleBajieIdeating(current, event) {
    if (event.type !== "bajie.product_idea.created")
        return stay(current);
    return {
...
function handleTangsengAdjusting(current, event) {
    if (event.type !== "tangseng.adjustment.decided")
        return stay(current);
    return {
...
function handleHougeBuilding(current, event) {
    if (event.type !== "houge.implementation.completed")
        return stay(current);
    return {
...
function handleShasengTesting(current, event) {
    ...
    if (event.type !== "shaseng.release_gate.passed")
        return stay(current);
    return {
...
function handleBailongmaFeedback(current, event) {
...
function handleWaitingCustomerFeedback(current, event) {
    if (event.type === "bailongma.feedback.received") {
...
function handleReleaseReady(current, event) {
...
function handleCustomerDelivered(current, event) {
    if (event.type !== "retrospective.completed")
        return stay(current);
    return {
        next: withState(current, "tangseng_planning", event.artifactIds),
        commands: [
...
function handleRetrospectiveReady(current, event) {
    if (event.type !== "iteration.next_planned")
        return stay(current);
    if (event.approvalRequired) {
        return {
...
function startExecutionAfterFounderApproval(current, artifactIds) {
    return {
...
function withState(current, state, artifactIds) {
    return {
        ...current,
        state,
...
function stay(current, commands = []) {
    return {
        next: current,
        commands
    };
}
function previousAgentFor(owner) {
    if (owner === "bajie")
        return "tangseng";
    if (owner === "houge")
        return "bajie";
...
function stateForAgent(agentName) {
    if (agentName === "bajie")
        return "bajie_ideating";
    if (agentName === "houge")
        return "houge_building";
    if (agentName === "shaseng")
        return "shaseng_testing";
    if (agentName === "bailongma")
        return "bailongma_collecting_feedback";
