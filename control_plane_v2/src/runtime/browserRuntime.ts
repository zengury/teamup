type RoleState = "orchestrating" | "thinking" | "waiting" | "queued" | "listening" | "active" | "done";
...
export type RuntimeSnapshot = {
  ...
  agents: Array<{
    id: "tangseng" | "bajie" | "houge" | "shaseng" | "bailongma";
    name: string;
    role: string;
    model: string;
    temperament: string;
    ...
    blocker: string;
    output: string;
    color: "green" | "gold" | "red" | "blue" | "silver";
  }>;
  journey: {
    mode: string;
    demandType: string;
  ...
  deliveries: Array<{
    title: string;
    status: "draft" | "ready" | "delivered" | "queued";
    owner: string;
    summary: string;
    result: string;
  }>;
...
export class BrowserProjectRuntime {
  ...
  private materialsGranted = false;
  private buildCompleted = false;
  private testingPassed = false;
  private feedbackReceived = false;
  private founderQuestionAsked = false;
  private projectQuery = "";
  private founderMessages: RuntimeSnapshot["thread"] = [];
  private parsed: ParsedConversation;
  ...
  founderDirective(text: string) {
    const message = text.trim();
    if (!message) return this.getSnapshot();
    this.founderMessages.push({
      who: "你",
      role: "如来佛祖",
      tone: "founder",
...
function parseConversation(transcript: string): ParsedConversation {
...
function deriveStage(flags: {
  ...
  materialsGranted: boolean;
  buildCompleted: boolean;
  testingPassed: boolean;
  feedbackReceived: boolean;
}) {
  if (!flags.scopeApproved) return "planning" as const;
  if (!flags.materialsGranted) return "product" as const;
  if (!flags.buildCompleted) return "building" as const;
  if (!flags.testingPassed) return "testing" as const;
  if (!flags.feedbackReceived) return "feedback" as const;
  return "iteration" as const;
}

function buildSummary(parsed: ParsedConversation, stage: ReturnType<typeof deriveStage>) {
  const positionMap = {
  ...
  ] as RuntimeSnapshot["summary"];
}

function positionValue(stage: ReturnType<typeof deriveStage>) {
  return {
    planning: "第 2 / 7 阶段",
    product: "第 3 / 7 阶段",
    building: "第 4 / 7 阶段",
...
function stageMeter(stage: ReturnType<typeof deriveStage>) {
  return {
    planning: 29,
    product: 43,
    building: 58,
...
function buildDecisions(approvals: { scope: boolean; outreach: boolean }, materialsGranted: boolean) {
  const decisions: RuntimeSnapshot["decisions"] = [];
  if (!approvals.scope) {
    decisions.push({
      id: "d1",
      from: "唐僧",
...
function buildAgents(
  ...
  approvals: { scope: boolean; outreach: boolean },
  materialsGranted: boolean,
  buildCompleted: boolean,
  testingPassed: boolean
): RuntimeSnapshot["agents"] {
  const tangsengStatus: RoleState = stage === "iteration" ? "orchestrating" : "active";
  return [
    {
      id: "tangseng",
      name: "唐僧",
...
function feedbackReceivedForStage(stage: ReturnType<typeof deriveStage>) {
  return stage === "feedback" || stage === "iteration";
}

function buildJourney(parsed: ParsedConversation, stage: ReturnType<typeof deriveStage>): RuntimeSnapshot["journey"] {
  const currentKey =
    stage === "planning"
      ? "tangseng_planning"
      : stage === "product"
  ...
  return {
    ...
    states: order.map((item, index) => ({
      ...item,
      status:
        index < currentIndex ? "done" : index === currentIndex ? "current" : index === currentIndex + 1 ? "next" : "future"
    }))
  };
}

function buildOutcome(parsed: ParsedConversation, testingPassed: boolean, feedbackReceived: boolean): RuntimeSnapshot["outcome"] {
  return {
...
function buildDeliveries(parsed: ParsedConversation, stage: ReturnType<typeof deriveStage>): RuntimeSnapshot["deliveries"] {
...
function buildFeedbackScenario(
  parsed: ParsedConversation,
  materialsGranted: boolean,
  feedbackReceived: boolean
): RuntimeSnapshot["feedbackScenarios"]["prototype"] {
  return {
...
function buildMarketLoop(parsed: ParsedConversation): RuntimeSnapshot["marketLoop"] {
  return [
    {
      source: "销售前线",
